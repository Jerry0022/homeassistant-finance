 ...
 *
 * Events dispatched:
 *   fd-diagnostics-updated — bubbles the raw /diagnostics snapshot so
 *     siblings (e.g. fd-state-banner) can react without an extra API call.
 */
...
class FdDiagnostics extends HTMLElement {
  ...
  _summaryIcon(d) {
    ...
    if (d.cache_is_stale) return "warn";
    return "ok";
  }
  ...
  _renderRateLimit(d) {
    ...
    if (!rl) {
      return `<div class="section">
        ...
        <div class="kv">
          <dt>Status</dt>
          <dd class="ok"><span class="pill ok">OK</span> ${d.rate_limit_per_day ?? 4} Abfragen/Tag pro Bank</dd>
        </div>
      </div>`;
    }
    ...
    return `<div class="section">
      ...
      <div class="kv">
        ...
        <dt>Limit</dt>
        <dd>${d.rate_limit_per_day ?? 4} Live-Abfragen/Tag pro Bank (Enable Banking PSD2)</dd>
      </div>
    </div>`;
  }

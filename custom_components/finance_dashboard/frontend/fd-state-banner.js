 ...
 * (same shape consumed by fd-diagnostics; the panel shell forwards the
 * "fd-diagnostics-updated" event it receives from that widget here, so
 * this component makes no API calls of its own).
...
class FdStateBanner extends HTMLElement {
  ...
  _chooseState(d) {
    ...
    // Field name matches manager.get_refresh_status()/get_diagnostics()
    // on the current cache-staleness contract (6h threshold).
    if (d.cache_is_stale && d.has_cache) {

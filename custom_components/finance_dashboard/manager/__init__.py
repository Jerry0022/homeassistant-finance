...
from ..utils import count_uncategorized, mask_iban
...
class FinanceDashboardManager(RefreshMixin, PersistenceMixin):
    ...
    def get_diagnostics(self) -> dict[str, Any]:
        ...
        Staleness is taken from ``get_refresh_status`` (the single
        hardened source using ``_CACHE_STALE_THRESHOLD_SECONDS``) rather
        than a parallel threshold, so the diagnostics widget and the
        header agree on when the cache is considered stale.
        """
        ...
        return {
            **status,
            ...
            "cache_stale": status["cache_is_stale"],
